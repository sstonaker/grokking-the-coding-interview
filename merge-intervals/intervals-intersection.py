...
def merge(intervals_a, intervals_b):
    ...
    while i < len(intervals_a) and j < len(intervals_b):
        # check if intervals overlap and intervals_a[i]'s start time is within
        # the intervals_b time
        a_overlaps_b = intervals_a[i][start] >= intervals_b[j][start] and \
            intervals_a[i][start] <= intervals_b[j][end]

        # check if intervals overlap and intervals_b[j]'s start time is within
        # the intervals_a time
        ...
        # move next from the interval that is finishing first

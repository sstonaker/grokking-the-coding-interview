...
def min_meeting_rooms(meetings):
    ...
    for meeting in meetings:
        ...
        while len(min_heap) > 0 and meeting.start >= min_heap[0].end:
            heappop(min_heap)
        # add the current meeting to min heap
        heappush(min_heap, meeting)
        # all active meetings are in the min heap, so we need rooms for all
...
def main():
    print("Minimum meeting rooms required: " +
          str(min_meeting_rooms([Meeting(1, 4),
                                 Meeting(2, 5), Meeting(7, 9)])))
    print("Minimum meeting rooms required: " +
          str(min_meeting_rooms([Meeting(6, 7),
                                 Meeting(2, 4), Meeting(8, 12)])))
    print("Minimum meeting rooms required: " +
          str(min_meeting_rooms([Meeting(1, 4),
                                 Meeting(2, 3), Meeting(3, 6)])))

...
Explanation: There are six contiguous subarrays whose product is less than the
target.
Example 2:
...
def find_subarrays(arr, target):
    ...
    for right in range(len(arr)):
        ...
        while product >= target and left < len(arr):
            ...
            # the target therefore, all subarrays from left to right will have
            # a product less than the target too; to avoid duplicates, we will
            # start with a subarray containing only arr[right] and then extend
        temp_list = deque()

...
def circular_array_loop(arr):
    for i in range(len(arr)):
        is_forward = arr[i] >= 0  # if we are moving forward
        slow, fast = i, i

        # if slow or fast become '-1' this means we can't find cycle for this
        # number
        while True:
...
def main():
    print(circular_array_loop([1, 2, -1, 2, 2]))
    print(circular_array_loop([2, 2, -1, 2]))

...
def rotate(head, rotations):
    ...
    last_node.next = head  # connect the last node with head to make a cycle
    rotations %= list_length  # no need to do rotations more than the length
    skip_length = list_length - rotations
    last_node_of_rotated_list = head
    for i in range(skip_length - 1):
        last_node_of_rotated_list = last_node_of_rotated_list.next

    # last_node_of_rotated_list.next is pointed to sub-list of k ending nodes

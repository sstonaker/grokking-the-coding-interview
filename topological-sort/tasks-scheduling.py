...
the number of tasks and a list of prerequisite pairs, find out if it is
...
task '1' needs
to finish before '2' can be scheduled. One possible scheduling of tasks is:
[0, 1, 2]
Example 2:
...
def main():
    print(
        f"Is scheduling possible: {str(is_scheduling_possible(3, [[0, 1], [1, 2]]))}")
    print(
        f"Is scheduling possible: {str(is_scheduling_possible(3, [[0, 1], [1, 2], [2, 0]]))}")
    print(
        f"Is scheduling possible: {str(is_scheduling_possible(6, [[2, 5], [0, 5], [0, 4], [1, 4], [3, 2], [1, 3]]))}")
